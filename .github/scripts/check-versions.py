...
def collect_library(plugin_root: Path, product: str) -> dict[str, str]:
    ...
    template_props = plugin_root / "templates/avalonia-app/Directory.Build.props"
    template_version = first_property(template_props, "AvaloniaMvvmExpressVersion")
    if not template_version:
        fail(f"{template_props} has no AvaloniaMvvmExpressVersion")
    versions[str(template_props.relative_to(plugin_root)) + " AvaloniaMvvmExpressVersion"] = template_version
    return versions
...
def collect_extensions(plugin_root: Path) -> dict[str, str]:
    return {
        ...
        "extensions/visualstudio/AvaloniaMvvmExpressPackage.cs": read_regex(
            plugin_root / "extensions/visualstudio/AvaloniaMvvmExpressPackage.cs",
            r'InstalledProductRegistration\("Avalonia MVVMExpress",\s*"[^"]+",\s*"([^"]+)"\)',
        ),
    }
