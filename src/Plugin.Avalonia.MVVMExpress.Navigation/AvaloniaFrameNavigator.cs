...
using Plugin.Avalonia.MVVMExpress.Lifecycle;
...
/// <summary>
/// <see cref="IPageNavigator"/> that hosts views in a Avalonia <see cref="Frame"/>.
/// Core <see cref="NavigationStack"/> is the source of truth. View construction always runs on <see cref="IMainThread"/>.
/// </summary>
public sealed class AvaloniaFrameNavigator : IPageNavigator, IRouteResolver
{
    ...
    private readonly Stack<Control> _viewStack = [];
    ...
    public AvaloniaFrameNavigator Map<
    ...
    public AvaloniaFrameNavigator Map(Type viewModelType, Type viewType, string? route = null)
    {
    ...
    private async Task<Result> GoOnUiAsync(
        ...
        CancellationToken cancellationToken)
    {
        var currentView = AvaloniaVisualTree.CurrentView(Window);
    ...
    private Frame? ResolveFrame() => _frame is not null ? _frame() : AvaloniaVisualTree.CurrentFrame(Window);
    ...
    private Control CreateView(
        ...
        IReadOnlyDictionary<string, object>? query)
    {
        var view = _services?.GetService(viewType) as Control
