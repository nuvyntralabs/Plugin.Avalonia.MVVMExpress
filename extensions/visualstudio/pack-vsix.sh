...
dotnet restore "$root/AvaloniaMvvmExpress.VisualStudio.csproj" --nologo
dotnet build "$root/AvaloniaMvvmExpress.VisualStudio.csproj" -c Release --nologo --no-restore \
  -p:CreateVsixContainer=false \
  -p:DeployExtension=false \
  -p:GeneratePkgDefFile=false

props="$root/obj/AvaloniaMvvmExpress.VisualStudio.csproj.nuget.g.props"
if [ ! -f "$props" ]; then
...
cp "$vsdk/inc/stdidcmd.h" "$vsdk/inc/vsshlids.h" "$root/AvaloniaMvvmExpressPackage.vsct" "$pack/"
(
  cd "$pack"
  if [[ "${OS:-}" == "Windows_NT" ]] || uname | grep -qiE 'mingw|msys|cygwin'; then
    ./VSCT.exe -nologo AvaloniaMvvmExpressPackage.vsct AvaloniaMvvmExpressPackage.cto
  elif command -v wine >/dev/null 2>&1; then
    WINEDEBUG=-all wine VSCT.exe -nologo AvaloniaMvvmExpressPackage.vsct AvaloniaMvvmExpressPackage.cto
  else
    echo "wine is required on macOS to compile AvaloniaMvvmExpressPackage.vsct. Install wine, or pack on Windows CI." >&2
    exit 1
  fi
)
...
rm -rf "$stage"
mkdir -p "$stage"
cp "$root/bin/Release/net472/NuvyntraLabs.AvaloniaMVVMExpress.VisualStudio.dll" "$stage/"
cp "$repo/LICENSE" "$stage/"

cat > "$stage/NuvyntraLabs.AvaloniaMVVMExpress.VisualStudio.pkgdef" <<'EOF'
[$RootKey$\Packages\{e3f4a5b6-8b4d-4e91-9c2a-6f0d8e1b7a44}]
@="AvaloniaMvvmExpressPackage"
"InprocServer32"="$WinDir$\SYSTEM32\MSCOREE.DLL"
"Class"="NuvyntraLabs.AvaloniaMVVMExpress.VisualStudio.AvaloniaMvvmExpressPackage"
"CodeBase"="$PackageFolder$\NuvyntraLabs.AvaloniaMVVMExpress.VisualStudio.dll"
"AllowsBackgroundLoad"=dword:00000001

[$RootKey$\InstalledProducts\Avalonia MVVMExpress]
"Package"="{e3f4a5b6-8b4d-4e91-9c2a-6f0d8e1b7a44}"
"UseInterface"=dword:00000000
"ProductDetails"="Create Avalonia MVVMExpress apps and pages from Plugin.Avalonia.MVVMExpress.Templates."
"PID"="__VERSION__"

[$RootKey$\AutoLoadPackages\{adfc4e64-0397-11d1-9f4e-00a0c911004f}]
"{e3f4a5b6-8b4d-4e91-9c2a-6f0d8e1b7a44}"=dword:00000002

[$RootKey$\AutoLoadPackages\{f1536ef8-92ec-443c-9ed7-fdadf150da82}]
"{e3f4a5b6-8b4d-4e91-9c2a-6f0d8e1b7a44}"=dword:00000002

[$RootKey$\Menus]
"{e3f4a5b6-8b4d-4e91-9c2a-6f0d8e1b7a44}"=", Menus.ctmenu, 1"
EOF
python3 -c "
from pathlib import Path
import sys
...
" "$(native_path "$stage/NuvyntraLabs.AvaloniaMVVMExpress.VisualStudio.pkgdef")" "$version"

cat > "$stage/extension.vsixmanifest" <<EOF
<?xml version="1.0" encoding="utf-8"?>
<PackageManifest Version="2.0.0" xmlns="http://schemas.microsoft.com/developer/vsx-schema/2011">
  <Metadata>
    <Identity Id="nuvyntralabs.AvaloniaMVVMExpress.VisualStudio" Version="${version}" Language="en-US" Publisher="Nuvyntra Labs" />
    <DisplayName>Avalonia MVVMExpress for Visual Studio</DisplayName>
    <Description>Install Plugin.Avalonia.MVVMExpress.Templates and create Avalonia MVVMExpress apps and pages via dotnet new.</Description>
    <MoreInfo>https://github.com/nuvyntralabs/Plugin.Avalonia.MVVMExpress</MoreInfo>
    <License>LICENSE</License>
    <Tags>Avalonia;MVVM;templates;dotnet new;mvvmexpress</Tags>
  </Metadata>
  <Installation>
  ...
  <Assets>
    <Asset Type="Microsoft.VisualStudio.VsPackage" Path="NuvyntraLabs.AvaloniaMVVMExpress.VisualStudio.pkgdef" />
  </Assets>
</PackageManifest>
EOF

cp "$pack/AvaloniaMvvmExpressPackage.cto" "$stage/"

if [ -z "$out_vsix" ]; then
  out_vsix="$root/../dist/nuvyntralabs.AvaloniaMVVMExpress.VisualStudio.${version}.vsix"
fi
...
version = "$version"
identity = "nuvyntralabs.AvaloniaMVVMExpress.VisualStudio"
description = (
    "Install Plugin.Avalonia.MVVMExpress.Templates and create Avalonia MVVMExpress "
    "apps and pages via dotnet new."
)
payload = [
    "extension.vsixmanifest",
    "LICENSE",
    "AvaloniaMvvmExpressPackage.cto",
    "NuvyntraLabs.AvaloniaMVVMExpress.VisualStudio.dll",
    "NuvyntraLabs.AvaloniaMVVMExpress.VisualStudio.pkgdef",
]
extension_dir = r"[installdir]\Common7\IDE\Extensions\nuvyntralabs\AvaloniaMVVMExpress"
    ...
    "packages": [
        {
            ...
            "localizedResources": [
                {
                    "language": "en-US",
                    "title": "Avalonia MVVMExpress for Visual Studio",
