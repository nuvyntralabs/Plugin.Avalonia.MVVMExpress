...
{
    public static readonly Guid CommandSet = new("f4a5b6c7-9c5e-4f02-8d3b-7e1c9a0f6b55");
    ...
    private static async Task CreateAppAsync(AsyncPackage package)
    {
        ...
        var parent = Prompts.AskFolder("Create the Avalonia MVVMExpress app in this folder");
        if (parent is null)
        {
            return;
        }
    ...
    private static async Task AddPageAsync(AsyncPackage package)
    {
        ...
        var outputDir = Prompts.AskFolder("Add the Avalonia MVVMExpress page in this folder", startFolder);
