...
public sealed class TemplateSmokeTests
{
    ...
    public void Template_ContainsExpectedFiles()
    {
        ...
        var template = Path.Combine(root, "templates", "avalonia-app");
        ...
        Assert.True(File.Exists(Path.Combine(template, "App1", "App.axaml.cs")));
        Assert.True(File.Exists(Path.Combine(template, "App1.Core", "HomeViewModel.cs")));
        Assert.True(File.Exists(Path.Combine(template, "App1", "Pages", "HomePage.axaml")));
    ...
    {
        var root = FindRepoRoot();
        var source = Path.Combine(root, "templates", "avalonia-app");
    ...
    private static string ReplacePackages(string csproj, string projectDir, string repoRoot)
    {
        ...
        csproj = Regex.Replace(
            csproj,
            """\s*<PackageReference Include="Plugin\.Avalonia\.MVVMExpress\.Core" Version="\$\(AvaloniaMvvmExpressVersion\)" />""",
            $"""{Environment.NewLine}    <ProjectReference Include="{Ref("Plugin.Avalonia.MVVMExpress.Core")}" />""");
        csproj = Regex.Replace(
            csproj,
            """\s*<PackageReference Include="Plugin\.Avalonia\.MVVMExpress\.Pagination" Version="\$\(AvaloniaMvvmExpressVersion\)" />""",
            $"""{Environment.NewLine}    <ProjectReference Include="{Ref("Plugin.Avalonia.MVVMExpress.Pagination")}" />""");
        csproj = Regex.Replace(
            csproj,
            """\s*<PackageReference Include="Plugin\.Avalonia\.MVVMExpress\.Testing" Version="\$\(AvaloniaMvvmExpressVersion\)" />""",
            $"""{Environment.NewLine}    <ProjectReference Include="{Ref("Plugin.Avalonia.MVVMExpress.Testing")}" />""");
        return csproj;
    }
